...
contract CarbonOracle is ChainlinkClient, ConfirmedOwner {
    ...
    event ProjectVerificationRequested(string indexed projectId, bytes32 requestId, address requester);
    ...
    function fulfill(
        ...
        emit ProjectVerificationReceived(projectId, verified, verification.verifiedAmount);
    }
     ...
     * @param projectId Project identifier
     */
    function getVerificationStatus(string memory projectId) 
        public 
        view 
        returns (
            bool isVerified,
     ...
     */
    function isVerificationCurrent(string memory projectId, uint256 maxAge) 
        public 
        view 
        returns (bool) 
    {
        ProjectVerification memory verification = verifications[projectId];
        return verification.lastUpdate > 0 && 
     ...
     */
    function setProjectAuthorization(string memory projectId, bool authorized) 
        public 
        onlyOwner 
    {
    ...
    function cancelRequest(bytes32 requestId) public onlyOwner {
        _cancelChainlinkRequest(requestId, fee, this.fulfill.selector, block.timestamp);
    }
     ...
     * @param projectId Project identifier
     */
    function isProjectAuthorized(string memory projectId) 
        public 
        view 
        returns (bool) 
    {
